...
public class UploadFileUtils {
	 ...
	 * 파일삭제
	 * 
	 * 보조 - 업로드시 날짜폴더생성, 이미지파일 여부체크-썸네일 생성작업
	 */

	// 기능 : 파일 업로드
	// 리턴타입 : String (업로드파일명)
	// 파라미터 : uploadPath (서버측의 업로드되는 기본폴더명) //업로드 파일 위치
	...
	public static String uploadFile(String uploadFolder, MultipartFile multipartFile ) {   
		...
		String uploadFolderPath = getFolder(); // "운영체제별 구분자를 이용하여" "2022\02\18"

		//  C:\\Dev\\upload\\2022\\01\\19
		File uploadPath = new File(uploadFolder, uploadFolderPath);  
		
		// uploadPath객체가 관리하는 폴더가 존재 유무 체크.    예>  "C:\\Dev\\upload" "2022/02/18"
		if(uploadPath.exists() == false) {
			...
			try {
				// "C:\\Dev\\upload" "2022\\02\\18" + 유일한 파일명
		...
		return uploadFileName;    //"2022\\02\\18" + 유일한 파일명   //pro_img로 넘어간다.
	}
	
	
	// 파일 출력 하는 기능: 클라이언트에게 byte[]로 보내는 기능
