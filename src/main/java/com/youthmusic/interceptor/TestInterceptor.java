...
// 인터셉터 클래스가 동작하기 위하여 servlet_context.xml 파일에서 설정작업을 해야한다.
public class TestInterceptor extends HandlerInterceptorAdapter {
	
	/*
	 - preHandle()?
	 	컨트롤러가 호출되기 전에 실행하는 특징. 예>/member/mypage, /order/orderinfo, /cart/cartlist
	 	컨트롤러의 매핑주소의 메서드가 실행이전에 처리해야 할 작업 또는 클라이언트로부터 넘어온 요청정보를 가공하거나 추가하는 경우에 작업.
	 	리턴값? boolean
	...
	/*
	 - postHandle()?
	 	...
	 	- 적용중인 인터셉터가 여러개인 경우 postHandle()역순으로 호출됨.
	...
	/*
	 - afterCompletion()?
	 	- 매핑주소의 메서드가 동작되고, 해당 뷰(View)가 최종결과를 생성하는 작업이 왼료된 후에 실행
	 	- 요청처리중에 사용한 리소스를 반환작업이 적당하다.
	 	preHandle()메서드에서 리턴값이 false인 경우 실행되지 않음.
	 	- 적용중인 인터셉터가 여러개인 경우 postHandle()역순으로 호출됨.
	 	- 비동기적 요청처리시에 호출되지 않음.
	 */
