											...
											<table id="example2"
												...
												<thead>
													<tr role="row">
														...
														<th class="sorting" tabindex="0" aria-controls="example2"
															rowspan="1" colspan="1"
															aria-label="CSS grade: activate to sort column ascending">재고</th>
														<th class="sorting" tabindex="0" aria-controls="example2"
															rowspan="1" colspan="1"
															aria-label="CSS grade: activate to sort column ascending">진열</th>
														<th class="sorting" tabindex="0" aria-controls="example2"
															rowspan="1" colspan="1"
															aria-label="CSS grade: activate to sort column ascending">수정</th>
														<th class="sorting" tabindex="0" aria-controls="example2"
															rowspan="1" colspan="1"
															aria-label="CSS grade: activate to sort column ascending">삭제</th>
													</tr>
												</thead>
												<tbody>
													...
													<c:forEach items="${productList }" var="productVO" varStatus="status">
														<tr role="row"
															...
															<td><fmt:formatDate value="${productVO.pro_loaddate }" pattern="yyyy-MM-dd" /></td>
															<td><input type="text" value='<c:out value="${productVO.pro_price }"></c:out>'></td>
															<td><input type="text" value='<c:out value="${productVO.pro_stock }"></c:out>'></td>		
															<td>
																<input type="checkbox" value="<c:out value="${productVO.pro_ok_buy }"></c:out>" 
																	<c:out value="${productVO.pro_ok_buy == 'Y' ? 'checked' : '' }"></c:out>>
															</td>
															<td><input type="button" name="btnProductEdit" value="수정" data-pro_num='<c:out value="${productVO.pro_num }"></c:out>'></td>
										...
										<div class="col-sm-5 dataTables_info">
											...
											<form id="searchForm" action="/admin/product/productList" method="get">
												<select name="type">
													...
													<option value="C"
														<c:out value="${pageMaker.cri.type eq 'C'? 'selected':'' }" />>제조회사</option>
...
<script>
	$(document).ready(function(){
		...
		$(".paginate_button a").on("click", function(e){
			e.preventDefault(); // <a href="">기능 취소

			// 기존페이지에서 사용자가 선택한 번호페이지로 이동
			actionForm.find("input[name='pageNum']").val($(this).attr("href"));
			actionForm.submit();	// <-- 값이 잘 넘어가는지 확인후 풀기
		...
		$("#btnCheckDelete").on("click", function(){
			...
			let isDelete = confirm("선택할 상품을 삭제하겠습니까?");

			if(!isDelete) return;

			//데이타 행에서 체크된 상품코드, 상품이미지

			// 자바스크립트 배열
			let pro_numArr = []; // 상품코드 배열
			let pro_imgArr = []; // 상품 이미지 배열
			let pro_uploadpathArr = []; // 날짜 폴더 배열
			...
			$.ajax({
				...
				success: function(data){
					if(data == "success"){
						alert("선택된상품이 삭제 되었습니다.");

						// 리스트주소 이동 또는 선택된 행을 동적으로 삭제
						// location.href= "주소";
		...
		// 수정버튼
		$("input[name='btnProductEdit']").on("click", function() {
			
			
			let pro_num = $(this).data("pro_num");

			/*
			let url= "/admin/product/productEdit?pro_num=" + pro_num;
			location.href = url;
			*/
