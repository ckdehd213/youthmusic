...
<body class="hold-transition skin-blue sidebar-mini">
													...
													<c:forEach items="${orderList }" var="orderVO"
														...
														<tr role="row"
															...
															<td><input type="checkbox" class="check"
																value='<c:out value="${orderVO.ord_num }"></c:out>'>

															</td>
															<td class="sorting_1"><fmt:formatDate
																	value="${orderVO.ord_date }"
																	pattern="yyyy-MM-dd mm:hh" /></td>
															<td><a class="move"
																href="<c:out value="${orderVO.ord_num }"></c:out>">
																	${orderVO.ord_num } </a></td>
															<td><input type="text"
																value='<c:out value="${orderVO.ord_name }" />'>
															</td>
															<td>주문방법</td>
															<td><input type="text"
																value='<c:out value="${orderVO.ord_total_price }"></c:out>'></td>
															...
															<td><input type="button" name="btnOrderStateChange"
																value="적용" data-ord_num="${orderVO.ord_num }">
																<input type="button" name="btnOrderDetail" value="상세보기"
																data-ord_num="${orderVO.ord_num }"></td>
	...
	$(function(){
		...
		$(".check").on("click", function() {
			...
			$("#ckeckAll").prop("checked", this.checked);
		...
		$("#btnOrderStateAll").on("click", function() {
			...
			$.ajax({
				...
				dataType: 'text',
				data: {
					ord_numArr : ord_numArr,
					ord_stateArr : ord_stateArr
				 },
				success: function (data) {
					if(data == "success"){
						alert("선택된 항목의 주문상태가 변경되었습니다.")
					}
				}	
		...
		$("input[name='btnOrderStateChange']").on("click", function() {
			...
			let ord_num = $(this).data("ord_num");
			let ord_state = $(this).parent().parent().find("select[name='ord_state']").val();
			
			console.log(ord_num + "," + ord_state);
			
			//자바스크립트 배열
			let ord_numArr = []; //주문번호 배열
			ord_numArr.push(ord_num);
			...
			$.ajax({
				...
				success : function(data){
					if(data == "success"){
						...
						actionForm.attr("action", "/admin/order/checkDelete");
		...
		$("input[name='btnOrderDetail']").on("click", function() {
			...
			$("#orderlist tr.tr_detail").css("display", "none");
			...
			let url = "/admin/order/detailInfo?ord_num=" + ord_num;
			detailInfo.load(url);
		});
